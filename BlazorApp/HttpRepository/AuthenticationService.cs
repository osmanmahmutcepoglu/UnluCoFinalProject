...
namespace BlazorApp.HttpRepository
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public async Task<Response<AuthResponseDto>> Login(UserForAuthenticationDto userForAuthentication)
        {
            ...
            var result = JsonSerializer.Deserialize<Response<AuthResponseDto>>(authContent, _options);

            if (!authResult.IsSuccessStatusCode)
                return result;

            await _localStorage.SetItemAsync("authToken", result.Data.JWToken);
            await _localStorage.SetItemAsync("Mail", result.Data.Email);
